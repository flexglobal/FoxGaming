...
peerWss.on('connection', (ws, req) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (!target) {
      console.log(`[PeerJS] peer ${dst} not found for ${peerId}`);
      try { ws.send(JSON.stringify({ type: 'ERROR', payload: `Peer ${dst} not found` })); } catch {}
      return;
    }
  ...
  ws.on('close', (code, reason) => {
    ...
    peerClients.forEach((entry) => {
      try { entry.ws.send(JSON.stringify({ type: 'LEAVE', src: peerId })); } catch {}
    });
  });
